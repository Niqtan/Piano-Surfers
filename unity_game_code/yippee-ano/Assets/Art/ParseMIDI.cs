...
void ProcessMIDI()
        ...
        if (serialPort != null && serialPort.IsOpen)
        {
            try
            {
                if (serialPort.BytesToRead > 0) 
                {
                    ...
                    {
                        int packetHeader = serialPort.ReadByte();
                        
                        if (packetHeader == 0xAA) // Our scoring identifier marker
                        {
                            int judgmentTierID = serialPort.ReadByte();
                            sbyte timingErrorMs = (sbyte)serialPort.ReadByte(); // Cast to signed 8-bit integer

                            // PROCESS HARDWARE CALCULATED MULTIPLIERS DIRECTLY
                            GiveGameFeedback(judgmentTierID, timingErrorMs);
                        }
                    }
                }
            }
    ...
    private void GiveGameFeedback(int tier, int errorMs)
    {
        if (tier == 1) {
            Debug.Log($"🎯 PERFECT! Calculated on Hardware! Error: {errorMs}ms");
            // Multiply UI text elements, trigger particles...

        }
        else {
            Debug.Log("❌ MISS / WRONG NOTE! Processing reset sequence markers.");
        }
    }
    void OnDisable()
    {
